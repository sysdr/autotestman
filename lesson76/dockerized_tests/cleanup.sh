#!/usr/bin/env bash
# Lesson 76 — stop Docker workloads, prune unused objects, clear local caches.
set -euo pipefail
...
echo "[cleanup] Lesson 76 — workspace: ${ROOT}"
echo "[cleanup] Lesson directory (parent): ${LESSON_DIR}"

echo "[cleanup] Stopping Docker containers (if any)..."
if command -v docker >/dev/null 2>&1; then
  if docker info >/dev/null 2>&1; then
    for id in $(docker ps -q --filter ancestor=uqap-tests:lesson76 2>/dev/null || true); do
